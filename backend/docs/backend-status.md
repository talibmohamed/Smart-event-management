...
- Structured agenda tracks and sessions with timezone-aware event scheduling
...
- Event create/update support optional `event_end_date`, `timezone`, and `agenda_tracks`
- Session ordering is `starts_at ASC`, then `sort_order ASC`
- Same-track agenda session overlap is rejected; cross-track overlap is allowed
