...
- `GET /api/auth/me` and paginated list endpoints such as `GET /api/events` and `GET /api/admin/users` do not include `message`
...
- Query params:
  ...
  - `q` optional string; searches email, first name, and last name case-insensitively
  - `role` optional enum: `attendee`, `organizer`, `admin`
  - `status` optional enum: `active`, `suspended`
  - `sort` optional enum: `created_desc`, `created_asc`, `email_asc`, `email_desc`
- Success:
...
- Common errors:
  - `400 { "success": false, "message": "pageSize must be less than or equal to 50" }`
  - `400 { "success": false, "message": "Invalid user role filter" }`
  - `400 { "success": false, "message": "Invalid user status filter" }`
  - `401 { "success": false, "message": "Not authorized" }`
...
- Notes:
  - `lastLoginAt` is omitted because login timestamps are not currently tracked
  - Password hashes and reset tokens are never selected or returned
- Common errors:
  - `401 { "success": false, "message": "Not authorized" }`
  - `403 { "success": false, "message": "Access denied. Admins only" }`
...
- Notes:
  ...
  - Changing an organizer to attendee does not delete or reassign existing events
- Success: `200 { "success": true, "message": "User role updated successfully", "data": "adminUser" }`
- Common errors:
  - `400 { "success": false, "message": "Invalid user role" }`
...
- Notes:
  ...
  - Suspending a user does not cancel bookings, delete events, refund payments, or send email in this version
  - Suspended users are rejected by login and every authenticated route through the auth middleware
- Success: `200 { "success": true, "message": "User status updated successfully", "data": "adminUser" }`
- Common errors:
  - `400 { "success": false, "message": "Invalid user status" }`
  - `400 { "success": false, "message": "You cannot suspend your own account" }`
  - `401 { "success": false, "message": "Not authorized" }`
  - `403 { "success": false, "message": "Access denied. Admins only" }`
  - `404 { "success": false, "message": "User not found" }`
